...
from pdf import armar_pdf


## MEJORAR LA GUI

## PRUEBA MANUAL CON DATA REAL
## PRUEBAS UNITARIAS EN PYTHON 
## ENCUESTA DE USABILIDAD 

## ENCUESTA CALIDAD MIN 10p 
...
class interfaz:
    ...
    def __init__(self):
        ...
        pygame.init()
        self.window = pygame.display.set_mode((515, 560))
        ...
        riff_1 = fuente.render("º  Riff 0 ", True, (100, 100, 100))
        riff_2 = fuente.render("º  Riff 1 ", True, (100, 100, 100))
        riff_3 = fuente.render("º  Riff 2 ", True, (100, 100, 100))
        riff_4 = fuente.render("º  Riff 3 ", True, (100, 100, 100))
        riff_5 = fuente.render("º  Riff 4 ", True, (100, 100, 100))
        riff_6 = fuente.render("º  Riff 5 ", True, (100, 100, 100))
        riff_7 = fuente.render("º  Riff 6 ", True, (100, 100, 100))
        riff_8 = fuente.render("º  Riff 7 ", True, (100, 100, 100))
        riff_9 = fuente.render("º  Riff 8 ", True, (100, 100, 100))
        riff_10 = fuente.render("º  Riff 9 ", True, (100, 100, 100))
        procesando = fuente.render("PROCESANDO", True, (50, 50, 50))
        self.finalizado = fuente.render("¡FINALIZADO!", True, (50, 50, 50))
        ...
        self.lis_boton_grab = [self.dicc_boton['grabar'], self.dicc_boton['parar'], self.dicc_boton['generar'],
                               self.dicc_boton['uno'], self.dicc_boton['dos'], self.dicc_boton['tres'],
                               self.dicc_boton['cuatro'], self.dicc_boton['reduccion_uno'],
                               self.dicc_boton['reduccion_dos'], self.dicc_boton['reduccion_tres']]
        '''
        self.dicc_cargar = {
            "cargar_1": {
                "visible": False,
        ...
        self.list_iconos_cargar = [self.dicc_cargar['cargar_1'], self.dicc_cargar['cargar_2'],
                                   self.dicc_cargar['cargar_3'], self.dicc_cargar['cargar_4']]
        '''
        self.dicc_iconos = {
            "grabar": {
                "visible": True,
        ...
        self.list_ondas = [self.dicc_ondas['onda_1'], self.dicc_ondas['onda_2'], self.dicc_ondas['onda_3'],
                           self.dicc_ondas['onda_4'],
                           self.dicc_ondas['onda_5'], self.dicc_ondas['onda_6'], self.dicc_ondas['onda_7'],
                           self.dicc_ondas['onda_8'], self.dicc_ondas['onda_9'], self.dicc_ondas['onda_10']]
        self.dicc_basura = {
            "basura_1": {
                "visible": True,
                "objeto": icono_basura,
        ...
        self.list_basura = [self.dicc_basura['basura_1'], self.dicc_basura['basura_2'], self.dicc_basura['basura_3'],
                            self.dicc_basura['basura_4'],
                            self.dicc_basura['basura_5'], self.dicc_basura['basura_6'], self.dicc_basura['basura_7'],
                            self.dicc_basura['basura_8'], self.dicc_basura['basura_9'], self.dicc_basura['basura_10']]
        ...
        self.list_riff = [self.dicc_riff['riff_1'], self.dicc_riff['riff_2'], self.dicc_riff['riff_3'],
                          self.dicc_riff['riff_4'],
                          self.dicc_riff['riff_5'], self.dicc_riff['riff_6'], self.dicc_riff['riff_7'],
                          self.dicc_riff['riff_8'], self.dicc_riff['riff_9'], self.dicc_riff['riff_10'],
                          self.dicc_riff['procesando']]

        self.return_val = []
        self.que = queue.Queue()
        ## hilo para la generacion de la tablatura
    ...
    def f_hilo_pdf(data):
        ...
        # hacer una funcion para activar todo el hilo y pasarle el diccionario
        root = Tk()
        archivo = filedialog.asksaveasfilename(initialdir="/", title='Seleccione Ruta', defaultextension=".pdf")
        root.destroy()

        que = queue.Queue()
        hilo_pdf = threading.Thread(target=lambda q, arg1: q.put(armar_pdf.main(arg1)), args=(que, {'tablatura':data,'ruta':archivo}))
        hilo_pdf.start()
        hilo_pdf.join()
        terminar = que.get()
        return terminar
    ...
    def accion_grabar_conteo(self):  # hacer el conteo 3,2,1,parar
        ...
        self.segundo = self.segundo + interfaz.temporizador(1)  ## contador de 1 segundo
    ...
    def controlador_gui(self, iniciar):
        ...
        while iniciar:
            ...
            if bandera_procesar:
                ...
                # inicia el hilo del procesamiento mientras pinta la carga
                if una_vez:
                    self.t.start()
                    self.t.join()
            ...
            ## Hilo para generar el pdf
            if self.return_val != [] and una_vez:

                interfaz.f_hilo_pdf(self.return_val)
                una_vez = False
